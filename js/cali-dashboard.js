...
function renderCaliDash() {
  ...
  if (activeTrees.length) {
    treeCardsHtml = activeTrees.map(tree => {
      ...
      const nextTarget = tree.nextSkill ? `▶ <strong>${tree.nextSkill.n}</strong> — target: ${tree.nextSkill.target} reps` : `<span style="color:#f39c12;">🏆 Tree Complete!</span>`;
      return `<div class="cali-tree-card">
        <div class="cali-tc-header">
          <span class="cali-tc-icon">${tree.icon}</span>
          <span class="cali-tc-name">${tree.tree}</span>
        ...
        <div class="cali-tc-skills">
          ${tree.curSkill ? `<div class="cali-tc-cur">✓ ${tree.curSkill.n} <span class="cali-tc-target">(${tree.curSkill.target} reps)</span></div>` : ''}
  ...
  const nextTargetsHtml = pending.slice(0, 5).map(p => `
    <div class="cali-target-row">
      ...
      <div class="cali-target-info">
        <div class="cali-target-name">${p.name}</div>
  ...
  const recentHtml = recent.map(w => {
    ...
    return `<div class="cali-sess-row">
      ...
      <div class="cali-sess-name">${w.exercise}</div>
  ...
  const prsHtml = prEntries.slice(0, 8).map(([name, data]) => {
    ...
    return `<div class="cali-pr-row">
      <div class="cali-pr-name">${name}</div>
  ...
  const dormantHtml = dormantTrees.length ? dormantTrees.map(t =>
    `<div class="cali-dormant-item"><span>${t.icon}</span><div class="cali-dormant-name">${t.tree}</div></div>`
  ).join('') : '';
...
function caliMuscTap(m) {
  ...
  const exercisesHtml = muscleTrees.map(tree => {
    const rows = tree.levelData.map(lvl => {
      ...
      return `<div class="cmtap-ex-row">
        ...
        <div class="cmtap-ex-info">
          <div class="cmtap-ex-name">${lvl.n}</div>
    ...
    return `<div class="cmtap-tree-section">
      <div class="cmtap-tree-hdr">${tree.icon} ${tree.tree} <span class="cmtap-tree-pct" style="color:${col}">${tree.pct}%</span></div>
      ${rows}
    </div>`;
...
function renderCaliJourney() {
  ...
  const treeHTML = CALISTHENICS_TREES.map(tree => {
    ...
    const nextHtml = nextSkill
      ? `<div class="cj-next">📈 Next: <strong>${nextSkill.n}</strong> · ${nextSkill.target} ${nextSkill.t === 'hold' ? 'secs hold' : 'reps'}</div>`
      ...
      <div class="cj-tree${isActive ? ' active' : ' dormant'}">
        <div class="cj-tree-hdr">
          ...
          <div class="cj-tree-info">
            <div class="cj-tree-name">${tree.tree}</div>
            <div class="cj-tree-status">${isActive && curSkill ? curSkill.n : 'Not started — tap BW tab'}</div>
          </div>
